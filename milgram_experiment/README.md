# Milgram Silicon Subjects (Web)

This app combines your two existing RAG personalities:
- `jekyllhyde` from `jekyllandhyde/app/rag_engine.py`
- `femwife` from `app/femandhousewife/src/pico_chatgpt_bridge/rag_engine.py`

It keeps both original projects unchanged and runs in a separate app folder.

## Features
- `chat` mode: user chats with one selected persona
- `shock` mode: admin and receiver personas run per-level reasoning/reflection
